...
- 本文是供人工逐项确认的候选清单；所有方框保持未勾选，本文不写入 Profile、override、digest 或 reviewStatus。
...
- 审核时可直接在文末“人工决策记录”填写决定；未推荐属性不应通过此文档擅自加入权重。
...
以下 63 个候选未经过人工审核；表中的 confidence 与非零权重均照录 artifact。
...
以下表单逐角色留空。请在审核后填写；本文本身不会写入 override 或标记 reviewed。

### Character 1001 — 三月七·存护
