...
describe('全局导航配置', () => {
  it('集中定义固定中文名称与七个唯一图标', () => {
    ...
    expect(new Set(NAVIGATION_ITEMS.map((entry) => entry.iconKey)).size).toBe(7);
  });
  ...
  it('首页精确匹配，其他一级路由覆盖详情子路由', () => {
    ...
    expect(isNavigationItemActive('/rogue/du/blessings', item('rogue'))).toBe(true);
